...
<body>
    ...
    ?>
        ...
        <?php $result = $db->prepare('SELECT * FROM job_inspection_cat ');
        $result->bindParam(':userid', $res);
        $result->execute();
        for($i=0; $row = $result->fetch(); $i++){   ?>
        <a href="inspection_list.php?id=<?php echo $_GET['id'] ?>&cat_id=<?php echo $row['id']; ?>" >
    <button class="model-box" style="margin-top: 15px; border-color:#F50721;"><?php echo $row['name'] ?></button>
