   ...
   $result = $db->prepare("SELECT * FROM sms WHERE  date BETWEEN '$d1' AND '$d2'  ");
			...
			?>
                <tr>
                  ...
                  <td><?php	$result1 = $db->prepare("SELECT * FROM customer WHERE  id='$cus'  ");
				$result1->bindParam(':userid', $date);
                $result1->execute();
                for($i=0; $row1 = $result1->fetch(); $i++){ echo $row1['contact']; } ?></td>
				  
                  <td>
			<span class="badge bg-<?php if($row['action']=="SEND"){ echo "green"; }if($row['action']=="Failed"){ echo "red"; } ?>"><?php echo $row['action'];?></span>
